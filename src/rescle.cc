...
bool ResourceUpdater::SetIcon(const WCHAR* path, const LANGID& langId,
  ...
  if (file == INVALID_HANDLE_VALUE) {
    fwprintf(stderr, (WCHAR*)"Cannot open icon file '%S'\n", path);
    return false;
  }

  IconsValue::ICONHEADER& header = icon.header;
  if (!ReadFile(file, &header, 3 * sizeof(WORD), &bytes, NULL)) {
    fwprintf(stderr, (WCHAR*)"Cannot read icon header for '%S'\n", path);
    return false;
  }

  if (header.reserved != 0 || header.type != 1) {
    fwprintf(stderr, (WCHAR*)"Reserved header is not 0 or image type is not icon for '%S'\n", path);
    return false;
  }
  ...
  if (!ReadFile(file, header.entries.data(), header.count * sizeof(IconsValue::ICONENTRY), &bytes, NULL)) {
    fwprintf(stderr, (WCHAR*)"Cannot read icon metadata for '%S'\n", path);
    return false;
  }
  ...
  for (size_t i = 0; i < header.count; ++i) {
    ...
    if (!ReadFile(file, icon.images[i].data(), icon.images[i].size(), &bytes, NULL)) {
      fwprintf(stderr, (WCHAR*)"Cannot read icon data for '%S'\n", path);
      return false;
    }
  }
